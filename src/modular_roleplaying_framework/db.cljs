...
When dice penalties are inflicted upon a dice pool, you decrease the number of dice in the pool by an amount equal to the number given in the penalty. If you are applying a dice penalty to a dice pool that only contains a single die, reduce the size of that die a number of times equal to the number given in the penalty. When flat bonuses or penalties are applied to a roll, you roll your base dice pool as you normally would and add or subtract the bonus or penalty before comparing it to the target number. If a dice bonus is applied after you roll your dice but before the roll is resolved, then roll the bonus dice and take the highest number between the dice pool and your bonus dice. If a dice penalty is inflicted after you roll all of your dice but before the roll is resolved, then roll the penalty dice one at a time and compare the result to the dice pool dice that you rolled. The die that is numerically closest to the penalty die you rolled is removed from the roll, rounding unfavorably, and then roll the next penalty dice. If you have penalty dice remaining when there is only one die left in the dice pool you rolled, then roll the next penalty die and take the lower of the two. Continue this process until there are no penalty dice left."
           ...
           12001 {:id 12001
                  ...
                  :text {:overview "Skill checks should be made when a character’s actions have a reasonable chance of both failure and success and when failure would come at a cost. If it is impossible for a skill check to fail or to succeed, then there is nothing to check. If a skill check has a reasonable chance of both success and failure but there is no cost for the actions either way, then the character could just continue to try again until they succeed. Ultimately, it is up to the GM’s discretion when a skill check should be called for. When a skill check is called for, you will use your character’s stats to determine the check’s base dice pool. The base dice pool may then be modified in three ways which must occur in the following order: bonuses and penalties, splintering the dice pool, and combining or splitting dice. Once the base dice pool has been modified, then the dice are rolled and the results are compared to a target number which represents the difficulty of the actions you are taking. For example, say the dice pool you roll for a skill check is 3d6 and you roll a 1, 3, and 4. The result of the check would be the 4 and the other two numbers are ignored."
                         ...
                         :passive "Passive checks may be used by the GM to resolve situations where your character’s stats are relevant but where there are no actions that your character takes to resolve the situation. Checks to determine your character's level of knowledge on a topic or checks to notice something in your surroundings when you aren’t searching will commonly be made as passive checks. The GM will still determine a target number and choose which stats are most relevant, and you will still put together your base dice pool and add the bonuses and penalties. However, instead of modifying that dice pool and rolling those dice, you will add together the number of dice in the dice pool, the net value of the bonuses and penalties, and half of the size of the dice that are being rolled and use that as the result of your check. One exception is that the result of a passive check cannot exceed the size of your ability die combined with the net value of your flat bonuses and penalties. So, in a simple case, if your base dice pool was 2d6 and you had no bonuses or penalties, then the result would be determined by adding 2 for the number of dice to 3 for half the size of your dice for a total of 5. If the 2d6 had a flat bonus of +2, then that is added to the result as well as the cap on the check. So the result would be 7 with a maximum possible result of 8. Once the result is determined, it is compared to the target number determined by the GM as usual. Passive checks cannot be modified by splintering, combining, or splitting."}}
           12021 {:id 12021
                  :title "Encounters"
                  :type "text"
                  :text {:overview "An encounter is a scene that is centered around a goal and obstacle where many of your actions require skill checks. One of the clearest examples of an encounter is combat. During combat, the most basic goal is survival, the obstacle to that goal is the creatures or characters that are trying to kill you, and most of your actions will relate to attacking, defending, or position which all frequently require skill checks. Other examples of scenes that may be counted as encounters are chase scenes, social functions, survival, heists, and investigations. Encounters are the most structured portions of an adventure. Your GM may decide to run some encounters more casually just like they would any other part of the adventure, however, if they want to track the order of actions more carefully, then they may use the following structure."
           ...
           10 {:id 10
               :type "skillbility"
               :title "Coordination"
               :details "Coordination represents your character's physical strength, flexibility, and control. A creature with high coordination will hit harder and faster and perform be able to perform impressive feats of athleticism and acrobatics."}
           11 {:id 11
               :type "skillbility"
               :title "Reflexes"
               :details "Reflexes represents the speed and accuracy of your character's reactions. A creature with high reflexes will be able to defend themself from incoming attacks much more reliably."}
           12 {:id 12
               :type "skillbility"
               :title "Endurance"
               :details "Endurance represents how much of a beating they can take and their resistance to sickness, disease, and exhaustion. A creature with high endurance will manage to survive even the most extreme circumstances."}
           13 {:id 13
               :type "skillbility"
               :title "Exertion"
               :details "Exertion represents the strength of your character's sense of self. A creature with high exertion is capable of handling enormous amounts of magical energy without fear of being consumed by it."}
           14 {:id 14
               :type "skillbility"
               :title "Instinct"
               :details "Instinct represents the strength and reliability of your character's gut reactions. A creature with high instinct is very sensitive to the constantly shifting aether and are able to block out or reinforce the magic that touches their spirit."}
           15 {:id 15
               :type "skillbility"
               :title "Perseverance"
               :details "Perseverance represents your character's ability to maintain their sense of self in the face of doubt and temptation. A creature with high perseverance is able to absorb large amounts of foreign aethereal energy without losing themself to the chaos."}
           16 {:id 16
               :type "skillbility"
               :title "Concentration"
               :details "Concentration represents your character's ability to focus on a single task and avoid distractions. A creature with high concentration is able to accomplish more impressive feats with greater efficiency."}
           17 {:id 17
               :type "skillbility"
               :title "Recognition"
               :details "Recognition represents your character's attention to detail. A creature with high recognition will often be able to notice the small things that others find insignificant. They are attentive to their surroundings and usually have great memories."}
           18 {:id 18
               :type "skillbility"
               :title "Comprehension"
               :details "Comprehension represents your character's ability to absorb and retain information. A creature with high comprehension is better able to understand the underlying principles of that which they are learning often causing them to pick up skills quickly and can make them great at teaching."}
           19 {:id 19
               :type "skillbility"
               :title "Persuasion"
               :details "Persuasion represents your character's ability to sway the dispositions of others through charm, intimidation, or deception. A creature with high persuasion is able to get what they want from people one way or another."}
           20 {:id 20
               :type "skillbility"
               :title "Insight"
               :details "Insight represents how well your character picks up on the many unspoken facets of a social environment. A creature with high insight can often discern the true motives and character of those they are interacting with."}
           21 {:id 21
               :type "skillbility"
               :title "Connections"
